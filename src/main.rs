...
fn update_buffers<'a>(
    ...
    let mut zbuffer = zbuffer.lock().unwrap();
    let mut cbuffer = cbuffer.lock().unwrap();
...
fn render_frame<'a>(
    ...
    let mut l_buffer = buffer.lock().unwrap();
    let mut l_cbuffer = cbuffer.lock().unwrap();
    // let mut l_zbuffer = zbuffer.lock().unwrap();
    buffer_prev.copy_from_slice(&l_buffer);
    cbuffer_prev.copy_from_slice(&l_cbuffer);
    drop(l_buffer);
    drop(l_cbuffer);

    buffer.lock().unwrap().fill(' ');
    cbuffer.lock().unwrap().fill(ANSI_escape_code::color::RESET);
    zbuffer.lock().unwrap().fill(0.0);
    ...
    thread::scope(|s| {
        s.spawn(|| {
            for index in 0..points_axis_range.a {
                let point = points[index];
                let color = points_color[index];
                let luminance = if index & 1 == 0 {
                    axes_luminance.a.0
                } else {
                    axes_luminance.a.1
                };
        ...
        s.spawn(|| {
            for index in points_axis_range.a..points_axis_range.b {
                let point = points[index];
                let color = points_color[index];
                let luminance = if index & 1 == 0 {
                    axes_luminance.a.0
                ...
                let b_arc_buffer_ref = Arc::clone(&buffer);
                let b_arc_zbuffer_ref = Arc::clone(&zbuffer);
                let b_arc_cbuffer_ref = Arc::clone(&cbuffer);
        ...
        s.spawn(|| {
            for index in points_axis_range.b..points_axis_range.c {
                let point = points[index];
                let color = points_color[index];
                let luminance = if index & 1 == 0 {
                    axes_luminance.a.0
                ...
                let c_arc_buffer_ref = Arc::clone(&buffer);
                let c_arc_zbuffer_ref = Arc::clone(&zbuffer);
                let c_arc_cbuffer_ref = Arc::clone(&cbuffer);
    ...
    for index in 0..l_buffer.len() {
        let val = l_buffer[index];
...
fn main() {
    ...
    let cbuffer = Arc::new(Mutex::new(
            vec![ANSI_escape_code::color::RESET; (width * height).into()]
            ));
    ...
    let zbuffer = Arc::new(Mutex::new(
        vec![0.0; (width * height).into()]
        ));
